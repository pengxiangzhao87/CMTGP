...
function openWindowPanel(title, url, options) {
	options = options || {};
	if(options.size && options.size.indexOf("x") >= 0) {
		options.width = options.size.split("x")[0];
		options.height = options.size.split("x")[1];
	}
	$("#WindowPanel iframe").attr("src", url);
    $("#WindowPanel").window({
    	closed:false,
    	modal:true,
    	title: title || '增加',
    	...
    	height: options.height || 450,
    	onClose: function() {
    		$("#WindowPanel iframe").removeAttr("src");
    		//关闭popup
    		try {
    			$("#popupWindow").window("close");
    		} catch(err) {
